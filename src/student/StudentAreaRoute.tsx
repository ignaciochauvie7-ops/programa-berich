...
type Page = 'program' | 'profile'

function StudentAreaAuthenticated({ page }: { page: Page }) {
  ...
  if (!user) {
    const from = page === 'profile' ? '/programa/mi-perfil' : `/programa/${slug}`
...
export function StudentAreaRoute({ page }: { page: Page }) {
  if (page === 'program') {
    const { slug = BERICH_PROGRAM_SLUG } = useParams<{ slug?: string }>()
  ...
  return <StudentAreaAuthenticated page={page} />
}
